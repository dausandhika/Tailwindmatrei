...
const BerandaView = ({ubahCari, cariProduct, hasilCari, hasilFilter }) => {
   ...
   return (
      <div className="beranda dark:bg-black">
        <label className="input input-bordered dark:bg-slate-500 dark:text-white text-black-500 flex items-center gap-2">
          <input
            type="text"
            className="grow cari"
            placeholder="Search"
        ...
        <div className="grid grid-cols-3">
          {hasilFilter?.restaurants?.map((data) => (
            <div className="" key={data?.name}>
              <div className="card bg-base-100 dark:bg-slate-900 dark:text-white w-auto shadow-xl">
                <figure>
                  <img src={`https://restaurant-api.dicoding.dev/images/small/${data.pictureId}`} alt="Shoes" />
                </figure>
